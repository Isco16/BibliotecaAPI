...
namespace BibliotecaAPI.Controllers.V2
{
    ...
    public class LibrosController : ControllerBase
    {
        ...
        public async Task<IEnumerable<LibroDTO>> Get([FromQuery] PaginacionDTO paginacionDTO)
        {
            ...
            var librosPaginados = await queryable.OrderBy(x => x.Titulo).Paginar(paginacionDTO).ToListAsync();
            //var libros = context.Libros.ToListAsync();
            var librosDTO = mapper.Map<IEnumerable<LibroDTO>>(librosPaginados);
            return librosDTO;
        }

        [HttpGet("{id:int}", Name = "ObtenerLibrosV2")]
        [AllowAnonymous]
        [OutputCache(Tags = [cache])]
        ...
        public async Task<ActionResult>Post(LibroCreacionDTO libroCreacionDTO)
        {
            ...
            await outputCacheStore.EvictByTagAsync(cache, default); // Eliminar la cache de los libros, para que se actualice la cache con el nuevo libro creado.
        ...
        public async Task<ActionResult> Delete(int id)
        {
            ...
            {
                //return BadRequest($"Libro con id {id} no encontrado");
                return NotFound();
            }
